...
class LoggingRestClient<Rq : RequestInfo, Rs : Any>(
    ...
    override suspend fun call(request: Rq): Rs {
        ...
        logWriter.logRequest(operationName, request.requestId, request)

        return try {
            val response = delegate.call(request)
            logWriter.logResponse(operationName, request.requestId, response, elapsedMillis(startedAt))
            response
        } catch (throwable: Throwable) {
            logWriter.logFailure(operationName, request.requestId, elapsedMillis(startedAt), throwable)
            throw throwable
        }
    }
