...
/** Сервис удаления greeting-сущности. */
@Service
class SampleGreetingDeleteService(
    @Qualifier(SampleGreetingRestClientConstants.DELETE_CLIENT) restClient: RestClient<GreetingResponse, ApiErrorResponse>,
    json: Json,
) : SampleGreetingOperationService(restClient, json, SampleGreetingRestClientConstants.DELETE_ACTION) {

    fun deleteGreeting(name: String) = execute(name)
}
